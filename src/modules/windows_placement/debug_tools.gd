...
func update_instance_window_rect(window: Window, max_instances_count: int, title_bar_height: int = 30) -> void:
	var screen_rect: Rect2 = Rect2(DisplayServer.screen_get_usable_rect(DisplayServer.get_primary_screen()))
	...
	var origin: Vector2 = screen_rect.position + Vector2(
		wrap(instance_index, 0, cols) * width, 
		wrap(floor(instance_index / 2.0), 0, rows) * height)
